...
folder = r"\\192.168.1.6\效率工程部\AI专用知识库\公司业务语境知识库V1\待转化文档"
target_dir = r"\\192.168.1.6\效率工程部\AI专用知识库\公司业务语境知识库V1\00.待归档文件"

def save_and_copy(md_lines, title):
    md_content = "\n".join(md_lines)
    local_path = os.path.join(r"c:\Users\michael\.gemini\antigravity", f"{title}.md")
    ...
    if not os.path.exists(target_dir):
        ...
        except Exception as e: print(f"Warning: Cannot make target dir {target_dir}")
...
def process_qualification(path, filename):
    ...
    title_suffix = filename.replace('-任职资格.xlsx', '').replace('.xlsx', '')
    title = f"2026-效率工程部-任职资格-{title_suffix}"
    lines = [
        "---",
        f'title: "{title}"',
        "type: 职位说明书",
    ...
    lines.extend([
        ...
        "",
        "Q: 这份任职资格文档主要解决什么问题？",
        f"A: 本文档提供了{title_suffix}的任职资格模型，详细规定了各职级所需的“核心素质”、“专业知识”与“业务技能”。",
        ""
    ])
...
def process_jd(path, filename):
    ...
    title_suffix = filename.replace('-职位说明书.xlsx', '').replace('.xlsx', '')
    title = f"2026-效率工程部-JD-{title_suffix}"
    lines = [
        "---",
        f'title: "{title}"',
        "type: 职位说明书",
    ...
    lines.extend([
        ...
        "",
        "Q: 这份文档主要解决什么问题？",
        f"A: 本文档提供了{title_suffix}的具体JD要求与职责明细。",
        ""
    ])
...
def process_octopus(path, filename):
    title_suffix = filename.replace('-八爪鱼图.xlsx', '').replace('.xlsx', '')
    title = f"2026-效率工程部-脑图-{title_suffix}"
    ...
    lines.extend([
        ...
        "",
        "Q: 本文档提供了什么核心信息？",
        f"A: 本文档是{title_suffix}的八爪鱼图，展平提取了其中的视觉层级结构文本。",
        ""
    ])
...
def main():
    ...
    for f in files:
        ...
        try:
            if '任职资格' in f:
                process_qualification(path, f)
            elif '职位说明书' in f or 'JD' in f:
                process_jd(path, f)
            elif '八爪鱼图' in f or '脑图' in f:
                process_octopus(path, f)
            else:
                print(f"Skipping unknown type: {f}")
        except Exception as e:
